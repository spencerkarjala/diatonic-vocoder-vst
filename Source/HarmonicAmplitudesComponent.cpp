...
void HarmonicAmplitudesComponent::loadShaderFile(std::string& destination, const char* filename) {
  destination.clear();

  std::filesystem::path path = std::string("../../Source/").append(filename);
...
void HarmonicAmplitudesComponent::newOpenGLContextCreated() {
  ...
  this->loadShaderFile(mVertexShader, "vertex.glsl");
  this->loadShaderFile(mFragmentShader, "fragment.glsl");
